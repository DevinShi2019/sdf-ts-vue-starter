 ...
 * @Date: 2019-07-19 19:06:29
 * @LastEditTime: 2019-08-15 18:00:37
...
class Storage {
  ...
  setList (keys, values) {
    ...
    for (index = 0; index < keys.length; index++) {
      this.setItem(keys[index], values[index])
    }
  }
  /* *
...
LsyStorages.install = (Vue, options = {}) => {
  ...
  if (localStorage) {
    ...
    Vue._localStorage = _localStorage
  }
  if (sessionStorage) {
    Vue.prototype.$sessionStorage = _sessionStorage
    Vue._sessionStorage = _sessionStorage
  }
  if (cookie) {
